...
maestro-messaging-rabbitmq    RabbitMQ transport.     [no tests]
...
maestro-admin / -admin-client Dashboard + event publisher.  [no tests]
...
- A module coverage gate fails the build if a `maestro-*` module with production
  code has no tests, with an explicit allowlist for the four that still don't.
...
want measurements or an SPI change rather than a quick patch.
